...
#define INPUT_STRING_LEN_BIG 50
...
typedef struct node {
  char name[INPUT_STRING_LEN_BIG];
  int IorC;
  int char_num;
...
typedef struct list {
  char table_name[INPUT_STRING_LEN_BIG];
  Node *head;
  Node *tail;
} Linkedlist;
...
void GetValue(int *comma, char *real);
int atoi_my(char *str);
bool IsEmpty(int i);
void Init_brackets();
int BCS(char *str, int *end);
...
int UpdateDecideType(char *value1, int *sign, int *comma);
...
int main() {
  ...
  while (!final) {
    ...
    int commands_id = 0, quo = 0, mistake = 0 ;
    ...
    while (scanf("%c", &commands[commands_id]) != EOF) {//输入 start
      if (commands[commands_id] == '\n') {  ///去掉换行符
        if(quo == 1){
          mistake = 1 ;
          quo = 0;
          if(commands[commands_id - 1] == ';'){
            break;
          }
        }
        commands[commands_id] = '\0';
      } else if(commands[commands_id] == '\'' && commands[commands_id - 1] != '\\'){
        if(quo == 1){
          quo = 0 ;
        } else {
          quo = 1 ;
...
void GetValue(int *comma, char *real){ ///real 一定一定是【】!
  char *this = strtok(NULL, " ");
  if(this == NULL) real = NULL;
  bool quo = false;
  char *end = this + strlen(this) - 1 ;
  for(char *start = this ; start <= end ; ++start){
    if(*start == '\'' && (start == this || *(start - 1) != '\\' )){
      quo = !quo;
    }
    if(*start == ',' && !quo){
      *start = ' ';
      *(this + strlen(this)) = ' ';
      char *this1 = strtok(this, " ");
      strcpy(real, this1);
      *comma = 1;
      break;
    }
...
void Init_node_insert(int this_list, int memory) {
  ...
  while (now != NULL) {
    if (now->char_num != memory) {
      for(int i = strlen(now->strings[memory]) - 1 ; i >= 0; --i){
        *(now->strings[memory] + i) = '\0';
      }
...
bool OperateInt(char *str1, char *operator, char *str2, int *sign) {
  if (strcmp(str1, "NULL") == 0 || strcmp(str2, "NULL") == 0) return false;
  if (strcmp(operator, "=") == 0) return (atoi_my(str1) == atoi_my(str2));
  if (strcmp(operator, "<") == 0) return (atoi_my(str1) < atoi_my(str2));
  if (strcmp(operator, ">") == 0) return (atoi_my(str1) > atoi_my(str2));
  if (strcmp(operator, "<>") == 0) return (atoi_my(str1) != atoi_my(str2));
  if (strcmp(operator, "<=") == 0) return (atoi_my(str1) <= atoi_my(str2));
...
int BCS(char *str, int *ptr_end) { ///修改掉字符串中的, ; )， 同时修改jump end值
...
int Create(int i) {///-1 : SYNTAX ERROR  0 : ERROR  1 succeed
  ...
  while (!end) {
    ...
    char *type = malloc(STR_LEN_BIG * sizeof (char));
    GetValue(&jump, type);
    ...
    if (strcmp(type, "INT") == 0 || strcmp(type, "INT,") == 0
      ...
      if(node->strings_len == 0) return -1;
      free(num);
    ...
    while (!jump && !end) {///用于处理not null primary key unique的循环
      char *possibility = malloc(STR_LEN_BIG * sizeof(char));
      GetValue(&jump, possibility);
      if (BCS(possibility,  &end) == -1) {
        return -1;
      }

      if (strcmp(possibility, "NOT") == 0) { ///NOT NULL
        char *null = malloc(STR_LEN_BIG * sizeof(char));
        GetValue(&jump, null);
        if (BCS(null, &end) == -1) return -1;
        ...
        char *key = malloc(STR_LEN_BIG * sizeof(char));
        GetValue(&jump, key);
        if (BCS(key,  &end) == -1) return -1;
        if (strcmp(key, "KEY") != 0) {
          return -1;
        } else {
          node->notnull = true;
...
int Insert(int list_id, int *this_list, int *memory) {
  ...
  if (*this_list == -1) return 0;///无法找到该表格,执行错误
  *memory = list[*this_list].head->char_num;
  char *values = strtok(NULL, " ");
  if(values == NULL || strncmp(values, "VALUES", 6) != 0) return -1;
  bool left_brackets_or = false;
  if(FindWord(values, '(') == 6){
    *(values + strlen(values)) = ' ';
    *brackets_move = '(';
    ++brackets_move;
    *(values + FindWord(values, '(')) = ' ';
    left_brackets_or = true;
    char *tem = strtok(values, " ");
    values = tem;
  }
  ...
  while (!end && now != NULL) {
    ...
    char *values_real1 = malloc(STR_LEN_BIG * sizeof(char));
    GetValue(&jump, values_real1);
    //if (BCS(values_real1, &jump, &end) == -1) return -1;
    int num1 = FindWord(values_real1, '\'');
    if (num1 == -1) { ///是数字和NULL的情况
      if(BCS(values_real1, &end) == -1) return -1 ;
      if(!jump && !end){
        char *hhh = strtok(NULL, " ");
        if(BCS(hhh,  &end) == -1) return -1;
        if(strcmp(hhh, "") != 0 || !end) return -1;
      }

      if(*values_real1 == '\0') return -1;
      if(strcmp(values_real1, "NULL") != 0){
        if (now->IorC != 1) return 0; ///类型错误 执行错误(如果是NULL就不管了）
        for(int i = 0 ; i < strlen(values_real1); ++i){
          if(!isdigit(*(values_real1 + i))) {
            if(!(i == 0 && *(values_real1 + i) == '-' && strlen(values_real1) != 1))
              return -1;//数字部分出现字母
          }
        }
      }
      ...
      while(1==1){//循环作用：拼接字符串
        ...
        if (num2 != -1) {//找到第二个引号
          ...
          *(values_real1 + strlen(values_real1)) = ' ';
          char *after = malloc(STR_LEN_BIG * sizeof (char));
          GetValue(&jump, after);
      ...
      if (now->IorC != 0) return 0; ///类型错误则执行错误
  ...
  while (now != NULL) {
    if (now->notnull && (strcmp(now->strings[now->char_num - 1], "NULL")) == 0) {
      return 0;                      ///检查本次输入中是否有notnull 但输入为NULL情况
    }
    if (now->unique) {
      for (int i = 0; i < now->char_num - 1; ++i) {
        if (strcmp(now->strings[i], now->strings[now->char_num - 1]) == 0) {
          if(strcmp(now->strings[now->char_num - 1], "NULL") != 0) return 0;///检查本次输入是否与前面相等，满足unique
        }
      }
    }
    now = now->next;
...
int ConditionDecideType(char *value1, int *sign) { ///找到字符串类型
  ...
  if (num1 == -1) { ///数字情况（无引号，疑似数字）
    for (int i = 0; i < strlen(value1); ++i) {
      if (!isdigit(*(value1 + i))) {
        if(!(i == 0 && *(value1 + i) == '-' && strlen(value1) != 1)){
          *sign = -1; ////其实应该是0
          break;
        }
      }
...
int Atom(int j, int result[], char *str) {
  ...
  if ((num1 == -1 && num2 != -1) || (num1 != -1 && num2 == -1)) {
    ...
    int flag1 = Condition(j, atom_result, next);
      ...
      if (strcmp(operator, "BETWEEN") == 0) { ///判断是否between
        ...
        if (IorC1) {
          for (int i = 0; i < list[j].head->char_num; ++i) {
            ...
            if(strcmp(AtomFieldOrConst(j, node1, value1, i), "NULL") == 0 ||
                strcmp(AtomFieldOrConst(j, node2, value2, i), "NULL") == 0 ||
                strcmp(AtomFieldOrConst(j,node3, value3, i), "NULL") == 0){
              atom_result[i] = -1;
            }
          }
          if (nag != 1)return nag;
...
int Conjunctable(int j, int result[], char *condition_start) {
  ...
  if (*start_move != '\0') {
    ...
    for (int i = 0; i < list[j].head->char_num; ++i) {
      result[i] = conjunctable_result1[i];
    }
  }
...
int Condition(int j, int result[], char *condition_start1) { //此处j为this_list return -1 语法错误 0 执行错误
  char condition_start[STR_LEN_BIG];
  memset(condition_start, 0 , sizeof condition_start);
  strcpy(condition_start, condition_start1);
  char *start_move = condition_start; ///开始走
  if (FindWordBack(condition_start, ';') == strlen(condition_start) - 1) {
...
int Delete(int *count, int list_id, int delete_index[]) {
  ...
  if (this_list == -1) return 0;  ///找到所在表格
    ...
    for(int i = 0 ; i < now->char_num ; ++i){
      if(now->strings[i] == NULL){
        for(int j = i ; j < now->char_num - 1 ; ++j){
          //for(int m = strlen(now->strings[j]) - 1 ; m >= 0; --m){ //清空原字符串
          // *((now->strings[j]) + m) = '\0';
          //}
          if(now->strings[j + 1] == NULL){
            now->strings[j] = NULL;
          } else{
            now->strings[j] = malloc(STR_LEN_BIG * sizeof(char));
...
int UpdateDecideType(char *value, int *sign, int *comma) { ///找到字符串类型
  char *value1 = malloc(STR_LEN_BIG * sizeof(char));
  strcpy(value1, value);
  int num1 = FindWord(value1, '\'');
  if (num1 == -1) { ///数字情况（无引号，疑似数字）
    if(FindWord(value1, ',') == strlen(value1) - 1){
      *comma = 1;
      *(value1 + FindWord(value1, ',')) = '\0';
    }
    for (int i = 0; i < strlen(value1); ++i) {
      if (!isdigit(*(value1 + i))) {
        if(!(i == 0 && *(value1 + i) == '-' && strlen(value1) != 1)){
...
int Update(int list_id, int *count) {
  ...
  *(where_start - 1) = '\0';
  ...
  memset(start_update, 0, sizeof start_update);
  ...
  while (!end) {
    ...
    int type = UpdateDecideType(value, &flag, &comma);
    if(flag != 1) return flag;
    if(comma == 0){
      if(strtok(NULL, " ") == NULL){
        end = 1;
    ...
    if (now->unique) {
      for (int i = 0; i < now->char_num; ++i) { //检查unique
        for (int j = 0; j < i; ++j) {
          if (strcmp(strings_copy[j], strings_copy[i]) == 0) {
            if(strcmp(strings_copy[i], "NULL") != 0){
              return 0;
            }
          }
        }
  ...
  while (!end) {
    ...
    char *value5 = strtok(NULL, " ");
    if (value5 == NULL) return -1;
    char value[STR_LEN_BIG];
...
int ID_sort_primary(int this_list, int id_amount, int *ptr_id[]) {
  ...
  for (int i = 0; i < id_amount; ++i) {
    array[i] = malloc(STR_LEN_BIG * sizeof(char));
    strcpy(array[i], now->strings[*ptr_id[i]]);
  }
  if (now->IorC) { //排序部分
    for (int j = 0; j < id_amount; ++j) {
      int min = atoi_my(array[j]), min_index = j;
      for (int i = j; i < id_amount; ++i) {
        if (atoi_my(array[i]) < min) {
          min = atoi_my(array[i]);
          min_index = i;
        }
      }
...
int ID_sort(int *ptr_id[COMMANDS_NUM_BIG], int id_amount, char *sort_name1, int this_list) {
  char sort_name[STR_LEN_BIG];
  memset(sort_name, 0, sizeof(sort_name));
  strcpy(sort_name, sort_name1);
  char *next = NULL;
  int end = 0, AorD = 1;
  ...
  if (!AorD) { ///判定倒序
    for (int i = 0; i < (id_amount - null_num) / 2; ++i) {
      char *temp = array[i];
      array[i] = array[id_amount - null_num - i - 1];
      array[id_amount - null_num - i - 1] = temp;

      int tem = *ptr_id[i];
      *ptr_id[i] = *ptr_id[id_amount - null_num - i - 1];
      *ptr_id[id_amount - null_num - i - 1] = tem;
    }
  }
...
//本次排序完成，考虑下一次排序
  ...
  for (int i = 0, j = 0; i < id_amount; ++i) {
    ...
    if ((i != id_amount - 1 && strcmp(array[i], array[i + 1]) == 0) ||
        id_amount != 1 && i != 0  && strcmp(array[i], array[i - 1]) == 0) {
      ++j;
    }
    if (j > 1 && (i == id_amount - 1 || strcmp(array[i], array[i + 1]) != 0 )) {
      int nag = 1;
      if (end) {
        nag = ID_sort_primary(this_list, j, ptr_id_next);
      } else {
        nag = ID_sort(ptr_id_next, j, next, this_list);
      }
      if (nag != 1) return nag;
      j = 0;
    }
...
int Select(int list_id) {
  ...
  *(sort_value + strlen(sort_value)) = ' ';
  ...
  if (strcmp(name, "*") == 0) {
    ...
    while (1 == 1) {
      int comma = 0;
      if (FindWordBack(name, ',') == strlen(name) - 1) {
        *(name + FindWordBack(name, ',')) = '\0';
        comma = 1;
      }
      Node *now = list[this_list].head;
      if(*name == '\0') return -1;
