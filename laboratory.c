...
int main() {
  ...
  while (!final) {
    ...
    for(int i = 0 ; i < COMMANDS_WORD_BIG; ++i){
      if(i != 0 && *(commands + i) == ',' && *(commands + i - 1) == ' '){
        for(int j = i - 1 ; j < COMMANDS_WORD_BIG - 1; ++j){
    ...
    for(int i = 0 ; i < COMMANDS_WORD_BIG; ++i){
      if(i < 5000 && *(commands + i) == ',' && *(commands + i + 1) != ' '){
        for(int j = 5000 ; j > i + 1; --j){
          *(commands + j) = *(commands + j - 1);
        }
        *(commands + i + 1) = ' ';
      }
...
int Create(int i) {///-1 : SYNTAX ERROR  0 : ERROR  1 succeed
  int primary_num = 0;
  bool return0 = false, left_brackets_or = false;  ///是否已找到（
...
int Insert(int list_id, int *this_list, int *memory) {
  ...
  if (*this_list == -1) {
    ...
    if(list[*this_list].head != NULL)*this_list = 0;
  }
  *memory = list[*this_list].head->char_num;
  ...
  while (!end && now != NULL) {
    ...
    if (num1 == -1) { ///是数字和NULL的情况
      ...
      int j = 0 ;
      for (int i = num1; i <= num2; ++i, ++j) { ///赋值循环操作
        *(now->strings[now->char_num] + j) = *(values_real1 + i);
      }
      *(now->strings[now->char_num] + j) = '\0';
...
int Update(int list_id, int *count) {
  ...
  *(where_start - 1) = '\0';
  ...
  while (!end) {
    ...
    for (int i = 0; i < now->char_num; ++i) {
      if (update_index[i] == 1) {
        for(int j = strlen(now->strings[i]) - 1; j >= 0; --j){
          *((now->strings[i]) + j) = '\0';
        }
        free(now->strings[i]);
        now->strings[i] = NULL;
        now->strings[i] = malloc(STR_LEN_BIG * sizeof(char));
